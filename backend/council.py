...
async def stage1_collect_responses(user_query: str, web_search: bool = False) -> List[Dict[str, Any]]:
    ...
    responses = await query_models_parallel(models, messages, web_search=web_search)

    # Retry once for models that did not respond
    failed_models = [model for model, response in responses.items() if response is None]
    retry_responses = {}
    if failed_models:
        retry_responses = await query_models_parallel(failed_models, messages, web_search=web_search)
